...
Current version: v3.113.0.
Filesystem predicates centralized in `gitmap/fsutil` (FileExists strict, FileOrDirExists loose, DirExists strict). cmd package migrated v3.113.0; other packages opportunistic. Adding a local `fileExists`/`dirExists` to cmd is forbidden — use `fsutil.*` to avoid redeclaration.
