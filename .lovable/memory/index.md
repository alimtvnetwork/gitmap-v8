...
Filesystem predicates centralized in `gitmap/fsutil` (FileExists strict, FileOrDirExists loose, DirExists strict). cmd package migrated v3.113.0; AST guard pinned in updatedebugwindows_source_test.go v3.114.0. Adding a local `fileExists`/`fileExistsLoose` to updatedebugwindows.go is forbidden — the AST test fails the build.
